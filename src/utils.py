...
def get_class_weights(df):
    """
    คำนวณน้ำหนักคลาสแบบผกผัน (Inverse Frequency) 
    พร้อมเพิ่มความสำคัญ (Boost) ให้กลุ่มเนื้อร้ายเพื่อเพิ่ม Recall
    """
    # 1. Map labels to indices
    if 'target' not in df.columns:
         targets = df['dx'].map(config.CLASS_TO_IDX).values
    ...
    # 3. Calculate Base Weights: Total / (Num_Classes * Count)
    total_samples = len(targets)
    weights = total_samples / (config.NUM_CLASSES * class_counts)
    
    # 4. Apply Malignant Boost (หากมีการเปิดใช้งานใน Config)
    # กลุ่มเนื้อร้าย: akiec(0), bcc(1), mel(4)
    if getattr(config, 'USE_MALIGNANT_BOOST', False):
        malignant_indices = [0, 1, 4] 
        boost_factor = getattr(config, 'MALIGNANT_BOOST_FACTOR', 1.0)
